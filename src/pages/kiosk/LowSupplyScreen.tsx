...
export const LowSupplyScreen: React.FC<LowSupplyScreenProps> = ({ initialConsumables }) => {
  ...
  if (loading) {
    return (
      <div className="kiosk-screen flex-1 flex items-center justify-center p-6 relative z-10">
        <Card className="kiosk-card w-full max-w-2xl p-10 sm:p-14 rounded-[3rem] border-0 shadow-2xl bg-white/95 backdrop-blur-xl text-center">
          <Loader2 className="w-14 h-14 animate-spin mx-auto text-[#f03861]" />
          <h2 className="kiosk-panel__title mt-6 text-4xl font-bold tracking-tight text-gray-950">Checking consumables...</h2>
          <p className="kiosk-panel__subtitle mt-3 text-xl text-gray-600">Please wait while we confirm the printer is ready.</p>
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="kiosk-screen flex-1 flex items-center justify-center p-6 relative z-10">
      ...
      >
        <Card className="kiosk-card overflow-hidden rounded-[3rem] border-0 shadow-2xl bg-white/95 backdrop-blur-xl">
          <div className="kiosk-panel__header bg-gradient-to-r from-[#fff7ed] via-[#fff4f0] to-[#fff1f4] px-6 py-8 sm:px-10 sm:py-12 border-b border-gray-100">
            <div className="flex items-start gap-5 sm:gap-6">
              <div className="kiosk-hero-icon kiosk-hero-icon--danger shrink-0 w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-red-50 text-[#f03861] flex items-center justify-center shadow-inner">
                <AlertTriangle className="w-10 h-10 sm:w-12 sm:h-12" />
              </div>
              <div className="space-y-3">
                <h2 className="kiosk-panel__title text-4xl sm:text-5xl font-extrabold tracking-tight text-gray-950">{headline}</h2>
                <p className="kiosk-panel__subtitle text-xl sm:text-2xl text-gray-700 leading-relaxed max-w-2xl">{message}</p>
              </div>
            </div>
          </div>

          <div className="px-6 py-8 sm:px-10 sm:py-10 space-y-6">
            <div className="kiosk-status-grid grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="kiosk-status-tile rounded-3xl border border-gray-100 bg-gray-50 p-5">
                <div className="kiosk-status-label text-sm font-semibold uppercase tracking-wider text-gray-500">Paper Remaining</div>
                <div className="kiosk-status-value mt-2 text-4xl font-bold text-gray-950">{paperRemaining}</div>
              ...
              <div className="kiosk-status-tile rounded-3xl border border-gray-100 bg-gray-50 p-5">
                ...
                <div className="kiosk-status-value mt-2 text-4xl font-bold text-gray-950">{tonerRemaining}</div>
                <div className="mt-1 text-sm text-gray-500">of {tonerCapacity || '-'}</div>
              </div>
            </div>

            <div className="kiosk-status-pill rounded-3xl border border-dashed border-[#f5a623]/30 bg-[#fffaf3] px-5 py-4 text-gray-700 text-lg">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Button
                size="xl"
                className="w-full flex-1 rounded-2xl h-16 text-xl font-bold"
